...
Mat quantize(Mat image, int bits) {
    Mat result (image.rows, image.cols, CV_8U);
    for(int row=0; row<image.rows; row ++) {
        for(int col=0; col<image.cols; col++) {
            Vec3b originalPixel = image.at<Vec3b>(row, col);
            Vec3b newPixel;
            for(int i=0; i<3; i++) {
                newPixel[i] = originalPixel[i] >> bits;
            }
            result.at<Vec3b>(row, col) = newPixel;
        }
    }
    return result;
}
...
int main(int argc, char *argv[]) {
    ...
    if (argc != 3) {
        cout << "Usage: " << argv[0] << " <image_file> <num_bits>" << endl;
        return -1;
    }
    ...
    Mat quantized_image = quantize(quantized_image, atoi(argv[2]));
    imshow("quantized image", quantized_image);

    Mat image_converted = convertToYUV420(quantized_image);
